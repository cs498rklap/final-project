...
postsServices.factory('Posts', function($http) {
	return {
		...
		getLike : function(searchField, searchTerm, sortField, sortDirection, limit, skip, count) {
			...
			return $http.get('http://localhost:3000/api/posts?find="'+searchTerm+'"&findField="'+searchField+'"&select={"__v": 0, "comments": 0}&sort={"'+sortField+'":"'+sortDirection+'"}&limit='+limit+'&skip='+skip+'&count='+count);
		},
